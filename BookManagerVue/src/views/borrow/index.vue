<template>
  <div class="app-container">
    ...
    <div class="filter-container" style="margin-bottom: 15px;opacity: 0.6;">
      ...
      <el-checkbox v-permission="['reader']" v-model="showRow" style="margin-left:20px;color:#fff;font-size: 20px;font-weight:500;">只看未还</el-checkbox>
      <!-- <el-radio v-permission="['reader']" v-model="radio" label="1" style="margin-left:20px;color:#1aac1a">备选项</el-radio> -->
    ...
    <el-table
        ...
        :default-sort="{ prop: 'borrowid', order: 'descending' }">
        
      <!-- <el-table-column
          fixed
          type="selection"
          width="55"
          v-if="(!showRow)||(scope.row.returntimestr === null || scope.row.returntimestr === '')">
      ...
      <el-table-column
          ...
          <template slot-scope="scope">
            <span  v-if="(!showRow)||(scope.row.returntimestr === null || scope.row.returntimestr === '')">
              {{scope.row.borrowid}}
            </span>
          </template>
      </el-table-column>
      ...
      <el-table-column
          label="还书时间">
          <template slot-scope="scope">
            <div style="display:inline-block;" v-if="(!showRow)||(scope.row.returntimestr === null || scope.row.returntimestr === '')">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-permission="['reader']" v-if="scope.row.returntimestr === null || scope.row.returntimestr === ''" @click="handleReturn(scope.row,scope.$index)" type="success" size="small">归还图书</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      this.$confirm('确定要删除该条记录吗?', '提示', {
        ...
        deleteBorrow(row).then(res => {
          if(res === 1) {
            ...
            addAdmin(this.id,'删除借阅信息','操作成功')
            // addAdmin(this.id,'删除借阅记录','操作成功')
            ...
            this.$message.error('删除记录失败,书籍未还')
            addAdmin(this.id,'删除借阅信息','删除记录失败,书籍未还')
          }
        })
      })
    },
      ...
      this.$confirm('确定要还书吗?', '提示', {
        ...
        returnBook(row.borrowid, row.bookid).then(res => {
          if(res === 1) {
            ...
            this.$message.error('还书成功,但由于已超过应还时间，记录违规一次！')
            this.handleCurrentChange(this.queryParam.page)
          }
          else {
