<template>
  <div class="app-container">
    <!-- 顶部功能 -->
    <div class="filter-container" style="margin-bottom: 15px; opacity:0.75;">
      <!-- 书名输入 -->
  ...
  methods: {
    ...
    handleCreate() {
      ...
      this.form = {
        ...
        bookprice: '35.5',
        booktypeid: 1,
        bookdesc: '效果惊人的基于CSS的Web设计示例。设计生气勃勃的文字使用技巧。机智的CSS技巧及如何使CSS适应未来需求。来自Web设计师和开发人员的CSS秘诀。高效使用图形以提高网页的下载速度。 Web视觉艺术设计的王者之书。',
        isborrowed: 0,
      ...
      if (this.roleIsAdmin) {
        ...
        this.$confirm('您确定要借书吗?', '提示').then(() => {
          borrowBook(this.id, row.bookid).then(res => {
            if(res === 1) {
              this.$message.success('恭喜您，借书成功！')
              this.handleCurrentChange(this.queryParam.page)
            }
            else if(res===2){
              this.$message.error('借书失败，您的账户已被封禁，请联系管理员解除！')
            } 
            else {
              this.$message.error('借书失败,该书被借阅中！')
            }
      ...
      if (this.formType === 0) {  // 添加记录
        addBookInfo(this.form).then(res => {
          if(res === 1) {
            ...
            addAdmin(this.id,'添加书籍'+this.form.bookname,'操作成功')
            // 跳转到末尾
            getCount().then(res => {
            ...
            addAdmin(this.id,'添加书籍'+this.form.bookname,'操作失败')
          }
        ...
        updateBookInfo(this.form).then(res => {
          if(res === 1) {
            ...
            addAdmin(this.id,'更新书籍'+this.form.bookname,'操作成功')
            this.handleCurrentChange(this.queryParam.page)
          } else {
            this.$message.error('更新书籍失败')
            addAdmin(this.id,'更新书籍'+this.form.bookname,'操作失败')
          }
          this.dialogFormVisible = false  // 关闭对话框
        })
      }
      ...
      this.$confirm('确定要删除该图书吗?', '提示', {
        ...
        deleteBookInfo(row).then(res => {
          if(res === 1) {
            ...
            addAdmin(this.id,'删除书籍'+row.bookname,'操作失败')
          }
        })
      })
    },
    ...
    handleDeleteSome() {
      this.$confirm('确定要删除这些图书吗?', '提示', {
        ...
        deleteBookInfos(items).then(res => {
          if(res > 0) {
            this.$message.success('删除' + res + '条图书成功')
            // console.log("傻逼测试开始")
            // console.log(items)
            // console.log(items.length)
            for(let i = 0; i < res; ++ i){
              console.log(items[i].bookname)
...
<style scoped>
  ...
  .app-container {
  background: url(../../../public/青空.jpg) no-repeat;
